import { lookupPoreValues, soilGroupFromBodenart, getHumusAdjustmentInterpolated } from './PoreLookUp';
import type { Horizont } from './HorizonQueries';

/** Returns Mächtigkeit in dm, or empty string if inputs are invalid. */
export function calcMaechtigkeitDm(tiefeOben: string, tiefeUnten: string): string {
...
export function rateAll(gpv: number, lk: number, fk: number, nfk: number): SoilRatings {
  return {
    gpv: rateGPV(gpv),
    lk: rateLK(lk),
    fk: rateFK(fk),
...
export function calcPoreCapacities(
  ...
  if (!isNaN(humusNum) && humusNum > 0) {
    ...
    if (!isNaN(nfk)) nfk += getHumusAdjustmentInterpolated(group, 'available', humusNum);
  }

  const skelett = parseFloat(skelettPct);
  const skelFactor = (!isNaN(skelett) && skelett >= 0) ? (100 - skelett) / 100 : 1;
  ...
  return {
    gpv_pct: fmt(gpv),   gpv_lm2: fmtLm2(gpv),
    lk_pct:  fmt(lk),    lk_lm2:  fmtLm2(lk),
    fk_pct:  fmt(fk),    fk_lm2:  fmtLm2(fk),
    nfk_pct: fmt(nfk),   nfk_lm2: fmtLm2(nfk),
  };
}
