...
import LabeledDropdownField, { type LabeledOption, type LabeledSection } from '@/components/LabeledDropdownField';
...
const WITTERUNG_OPTIONS: LabeledOption[] = [
  ...
  { code: 'WT3', label: 'keine Niederschläge innerhalb der letzten 24 Stunden' },
  { code: 'WT4', label: 'regnerisch mit nicht sehr starken Niederschlägen innerhalb der letzten 24 Stunden' },
  { code: 'WT5', label: 'stärkere Regenfälle seit mehreren Tagen oder Starkregen innerhalb der letzten 24 Stunden' },
...
const NUTZUNG_SECTIONS: LabeledSection[] = [
  {
    ...
    data: [
      ...
      { code: 'GD', label: 'Weide' },
      { code: 'GS', label: 'Streuobstwiese, Grünland mit Wert-, Energieholz oder Obstbäumen' },
  ...
  },
  {
    title: 'Ödland',
    data: [
      { code: 'O',  label: 'Ödland allgemein (naturnah, ungenutzt oder extensiv genutzt)' },
  ...
  },
  {
    ...
    data: [
      ...
      { code: 'NG', label: 'Kleingartenanlage, Haus- und Kleingarten, Nutzgarten, Gartenland' },
  ...
  },
  {
    ...
    data: [
      ...
      { code: 'VE', label: 'städtisch geprägte Fläche, Siedlung, Dorfanlage, Wohngebiet' },
...
const VEGETATION_SECTIONS: LabeledSection[] = [
  ...
  },
  {
    ...
    data: [
      ...
      { code: 'NPT', label: 'naturnahe Pflanzengesellschaften trockener Standorte' },
      { code: 'NPN', label: 'naturnahe Pflanzengesellschaften nasser Standorte' },
      { code: 'NPS', label: 'naturnahe Pflanzengesellschaften von Salzstandorten' },
 ...
 */
export default function AufnahmeForm({ initialData, onSave, calcGrundigkeit, horizonte = [], onNotizenFocus, onNotizenBlur }: Props) {
  ...
  const { control, setValue, watch, getValues } = useForm<FormData>({
    defaultValues: {
      ...
      effektiver_wurzelraum:   initialData.effektiver_wurzelraum != null ? String(initialData.effektiver_wurzelraum) : '',
    ...
    <View style={styles.section}>
      ...
      <View style={styles.formRow}>
        <View style={styles.halfField}>
          ...
          <Controller control={control} name="reliefpos" render={({ field: { onChange, value } }) => (
            <LabeledDropdownField value={value} options={RELIEFPOS_OPTIONS} placeholder="Auswählen…" onChange={onChange} />
          )} />
        </View>
        <View style={styles.halfField}>
      ...
      <View style={styles.formRow}>
        <View style={styles.halfField}>
          ...
          <Controller control={control} name="nutzung" render={({ field: { onChange, value } }) => (
            <LabeledDropdownField value={value} sections={NUTZUNG_SECTIONS} placeholder="Auswählen…" onChange={onChange} />
          )} />
        </View>
        <View style={styles.halfField}>
          <Text style={styles.fieldLabel}>Vegetation (19)</Text>
          <Controller control={control} name="vegetation" render={({ field: { onChange, value } }) => (
            <LabeledDropdownField value={value} sections={VEGETATION_SECTIONS} placeholder="Auswählen…" onChange={onChange} />
          )} />
        </View>
      </View>
      ...
      <View style={styles.formRow}>
        <View style={styles.halfField}>
          ...
          <Controller control={control} name="witterung" render={({ field: { onChange, value } }) => (
            <LabeledDropdownField value={value} options={WITTERUNG_OPTIONS} placeholder="Auswählen…" onChange={onChange} />
          )} />
        </View>
        <View style={styles.halfField}>
      ...
      <Controller control={control} name="notizen" render={({ field: { onChange, value } }) => (
        <TextInput style={[styles.input, localStyles.multiline]} placeholder="Freitext…"
          ...
          onFocus={onNotizenFocus} onBlur={onNotizenBlur} />
      )} />
      ...
      >
        ...
        <Controller control={control} name="hangneigung" render={({ field: { onChange, value } }) => (
          <TextInput style={styles.input} placeholder="z.B. 5°" placeholderTextColor={colors.primary + '66'} onChangeText={onChange} value={value} />
        )} />

        <Text style={styles.fieldLabel}>Reliefformtyp (14)</Text>
        <Controller control={control} name="reliefformtyp" render={({ field: { onChange, value } }) => (
          <TextInput style={styles.input} placeholder="z.B. Kuppe" placeholderTextColor={colors.primary + '66'} onChangeText={onChange} value={value} />
        )} />

        <Text style={styles.fieldLabel}>Mikrorelief (16)</Text>
        <Controller control={control} name="mikrorelief" render={({ field: { onChange, value } }) => (
          <TextInput style={styles.input} placeholder="z.B. eben" placeholderTextColor={colors.primary + '66'} onChangeText={onChange} value={value} />
        )} />

        <Text style={styles.fieldLabel}>Natürlicher Bodenabtrag (17a)</Text>
        <Controller control={control} name="nat_bodenabtrag" render={({ field: { onChange, value } }) => (
          <TextInput style={styles.input} placeholder="" placeholderTextColor={colors.primary + '66'} onChangeText={onChange} value={value} />
        )} />

        <Text style={styles.fieldLabel}>Künstlicher Bodenabtrag (17b)</Text>
        <Controller control={control} name="kuenstl_bodenabtrag" render={({ field: { onChange, value } }) => (
          <TextInput style={styles.input} placeholder="" placeholderTextColor={colors.primary + '66'} onChangeText={onChange} value={value} />
        )} />

        <Text style={styles.fieldLabel}>Anthropogene Veränderungen (21)</Text>
        <Controller control={control} name="anthropogene_veraend" render={({ field: { onChange, value } }) => (
          <TextInput style={styles.input} placeholder="" placeholderTextColor={colors.primary + '66'} onChangeText={onChange} value={value} />
        )} />

        <Text style={styles.fieldLabel}>Bodenoberfläche (22)</Text>
        <Controller control={control} name="bodenoberflaeche" render={({ field: { onChange, value } }) => (
          <TextInput style={styles.input} placeholder="" placeholderTextColor={colors.primary + '66'} onChangeText={onChange} value={value} />
        )} />

        <Text style={styles.fieldLabel}>Versiegelungsart (23)</Text>
        <Controller control={control} name="versiegelungsart" render={({ field: { onChange, value } }) => (
          <TextInput style={styles.input} placeholder="" placeholderTextColor={colors.primary + '66'} onChangeText={onChange} value={value} />
        )} />

        <Text style={styles.fieldLabel}>Regenwürmer (24)</Text>
        <Controller control={control} name="regenwuermer" render={({ field: { onChange, value } }) => (
          <TextInput style={styles.input} placeholder="" placeholderTextColor={colors.primary + '66'} onChangeText={onChange} value={value} />
        )} />

        <Text style={styles.fieldLabel}>Substratsystematische Einheit (61)</Text>
        <Controller control={control} name="substratsyst_einheit" render={({ field: { onChange, value } }) => (
          <TextInput style={styles.input} placeholder="" placeholderTextColor={colors.primary + '66'} onChangeText={onChange} value={value} />
        )} />

        <Text style={styles.fieldLabel}>Hydrogeniet. Moortyp (64)</Text>
        <Controller control={control} name="hydrogeniet_moortyp" render={({ field: { onChange, value } }) => (
          <TextInput style={styles.input} placeholder="" placeholderTextColor={colors.primary + '66'} onChangeText={onChange} value={value} />
        )} />

        <Text style={styles.fieldLabel}>Durchwurzelbarer Bodenraum (65)</Text>
        <Controller control={control} name="durchwurzelbarer_bodenraum" render={({ field: { onChange, value } }) => (
          <TextInput style={styles.input} placeholder="" placeholderTextColor={colors.primary + '66'} onChangeText={onChange} value={value} />
        )} />

        <Text style={styles.fieldLabel}>Wasserstand u. GOF (66)</Text>
        <Controller control={control} name="wasserstand_gof" render={({ field: { onChange, value } }) => (
          <TextInput style={styles.input} placeholder="" placeholderTextColor={colors.primary + '66'} onChangeText={onChange} value={value} />
        )} />

        <Text style={styles.fieldLabel}>Grundnässestufe (67)</Text>
        <Controller control={control} name="grundnaessestufe" render={({ field: { onChange, value } }) => (
          <TextInput style={styles.input} placeholder="" placeholderTextColor={colors.primary + '66'} onChangeText={onChange} value={value} />
        )} />

        <Text style={styles.fieldLabel}>Besond. Wasserverhältnisse (68)</Text>
        <Controller control={control} name="besond_wasserverh" render={({ field: { onChange, value } }) => (
          <TextInput style={styles.input} placeholder="" placeholderTextColor={colors.primary + '66'} onChangeText={onChange} value={value} />
        )} />

        <Text style={styles.fieldLabel}>Stau-/Haftnässestufe (69)</Text>
        <Controller control={control} name="stau_haftnaessestufe" render={({ field: { onChange, value } }) => (
          <TextInput style={styles.input} placeholder="" placeholderTextColor={colors.primary + '66'} onChangeText={onChange} value={value} />
        )} />

        <Text style={styles.fieldLabel}>Erosionsgrad (70)</Text>
        <Controller control={control} name="erosionsgrad" render={({ field: { onChange, value } }) => (
          <TextInput style={styles.input} placeholder="" placeholderTextColor={colors.primary + '66'} onChangeText={onChange} value={value} />
        )} />
      </CollapsibleSection>
      ...
      >
        ...
        <Controller control={control} name="grundigkeit" render={({ field: { value } }) => (
        ...
        <Text style={[styles.fieldLabel, { marginTop: 8 }]}>Effektiver Wurzelraum (cm)</Text>
        <Controller control={control} name="effektiver_wurzelraum" render={({ field: { onChange, value } }) => (
          <TextInput style={styles.input} keyboardType="number-pad" placeholder="z.B. 80"
            placeholderTextColor={colors.primary + '66'} onChangeText={onChange} value={value} />
        )} />
